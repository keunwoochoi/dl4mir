...
def data_gen(set_name, nsp_input, is_shuffle, is_infinite, batch_size):
    ...
    srcs = np.load(os.path.join(DIR_JAMENDO_NPY, '{}_x.npy'.format(set_name)))
    ys = np.load(os.path.join(DIR_JAMENDO_NPY, '{}_y.npy'.format(set_name)))
    ...
    while True:
        for batch_i in xrange(n_batch):
            ...
            for data_i in data_idxs:
                src = srcs[data_i]
                y = ys[data_i]
                if nsp_input is not None:
                    nsp_src = len(src)
                    offset = np.random.randint(high=nsp_src - nsp_input)
...
def main(model_name, exp_name='fma'):
    ...
    gen_valid = data_gen('valid', SR * 10, is_shuffle=False, is_infinite=False, batch_size=5)
    ...
    model.fit_generator(gen_train, steps_per_epoch, epochs=5,
                        ...
                        validation_steps=20 // valid_batch_size)

    print("Keunwoo: Training is done. Loading the best weights...")
    model.load_weights("{}_best_weights.h5".format(exp_name))
    gen_test = data_gen('test', None, is_shuffle=False, is_infinite=False, batch_size=1)

    print("         Evaluating...")
    scores = model.evaluate_generator(gen_test, 20)
